...
poses serve_pose = {-0.29, 0.21, -0.48, 0.0, 0.0, 0.0}; // -0.3, 0.22 -0.48
poses prehome_pose = {-0.3, 0.2, 0.15, 0.0, 0.0, 0.0};
// IK Transforms
poses grasp_frame_transform = {0.0, 0.02, 0.20, M_PI/2, 0.0, 0.0}; // {x, y, z, rx, py, yz} Orients gripper and open/close horizontal
...
mtc::Task UrControl::createTask()
{
    ...
    //interpolation_planner->setTimeout(gripper_timeout);
    //interpolation_planner->setMaxAccelerationScalingFactor(1.0);
    //interpolation_planner->setMaxVelocityScalingFactor(1.0);
    ...
    // ***Close Gripper <MoveTo>***
    {
        ...
        stage->setGoal("close");       
        //stage->setTimeout(gripper_timeout);
